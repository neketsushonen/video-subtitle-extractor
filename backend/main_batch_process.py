...
# Función que extrae subtítulos para un video
def extract_subtitles(video_file, video_directory, output_directory, subtitle_area):
    video_path = os.path.join(video_directory, video_file)
    output_path = os.path.join(output_directory, video_file.rsplit('.', 1)[0] + ".srt")
    
    # Verificar si el archivo de salida ya existe
    if os.path.exists(output_path):
        print(f"El archivo de subtítulos ya existe para {video_file}, saltando procesamiento.")
        return None
    # Crear el extractor de subtítulos
    se = SubtitleExtractor(video_path, subtitle_area)
    # Comenzar la extracción
    se.run()
    print(f"Subtítulos extraídos para {video_file} y guardados en {output_path}")


def process_videos_in_parallel(video_directory, output_directory, subtitle_area):
    # Obtener el número de núcleos disponibles
    num_cores = os.cpu_count()
    
    # Calcular la mitad de los núcleos
    max_workers = num_cores // 2  # Usar la mitad de los núcleos

    # Crear un ProcessPoolExecutor, que ejecutará las tareas en paralelo
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Recorremos los archivos en el directorio de videos y filtrar solo los .mp4
        video_files = [f for f in os.listdir(video_directory) if f.endswith('.mp4')]
        
        # Asignamos a cada archivo de video un proceso para la extracción de subtítulos
        futures = [
            executor.submit(extract_subtitles, video_file, video_directory, output_directory, subtitle_area)
...
if __name__ == '__main__':
    ...
    video_directory = "/Users/chunhaulai/Documents/workspace-personal/video-subtitle-extractor/downloads/"
    output_directory = "/Users/chunhaulai/Documents/workspace-personal/video-subtitle-extractor/downloads/"
    subtitle_area = (602,662,201,1073) # configurar desde gui.py
    process_videos_in_parallel(video_directory, output_directory, subtitle_area)
